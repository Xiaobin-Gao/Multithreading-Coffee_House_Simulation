...
public class Simulation {
	...
	public static volatile Queue<Order> orders = new LinkedList<Order>();;
	...
	public static List<SimulationEvent> runSimulation(
		...
		TableInfo tableInfo = new TableInfo(numTables);
		Machine grill = new Machine("Grill", FoodType.burger, machineCapacity);
		Machine frier = new Machine("Frier", FoodType.fries, machineCapacity);
		Machine star = new Machine("Star", FoodType.coffee, machineCapacity);
		...
		for(int i = 0; i < cooks.length; i++) {
			cooks[i] = new Cook("Cook" + i, 3, grill, frier, star);
			cooks[i].start();
		}
		...
		if (!randomOrders) {
			...
			for(int i = 0; i < customers.length; i++) {
				customers[i] = new Thread(
						new Customer("Customer " + (i+1), order, random.nextInt(3) + 1, tableInfo)
						);
			}
		}
		else {
	...
	private synchronized static Queue<Order> getOrderQueue() {
		return orders;
	}
	...
	public static void main(String args[]) throws InterruptedException {
		...
		int numCustomers = 50;
		int numCooks = 3; 
		int numTables = 10; // numTables = 0 => no customer can enter
		int machineCapacity = 15; // machineCapacity = 0 => no food can be made
